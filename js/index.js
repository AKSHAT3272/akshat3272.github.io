...
var logo = jQuery('.cd-header #cd-logo img');
var scrolled = false;
jQuery(window).scroll(function() {
    if (300 < jQuery(window).scrollTop() && !scrolled) {
        logo.addClass('smoly').animate({
            top: '0px'
        }); 
        scrolled = true; 
    }

    if (115 > jQuery(window).scrollTop() && scrolled) {
        logo.removeClass('smoly').css('top', '0px');
        scrolled = false;
